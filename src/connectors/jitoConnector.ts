...
export class JitoTransactionExecutor implements TransactionExecutor {
  ...
  public async executeAndConfirm(
    ...
    coloredDebug('Starting Jito transaction execution...');
    this.JitoFeeWallet = this.getRandomValidatorKey(); // Update wallet key each execution
    coloredInfo(`Selected Jito fee wallet: ${this.JitoFeeWallet.toBase58()}`);

    try {
      const fee = new CurrencyAmount(Currency.SOL, this.jitoFee, false).raw.toNumber();
      coloredInfo(`Calculated fee: ${fee} lamports`);
      ...
      coloredInfo('Sending transactions to endpoints...');
      const results = await Promise.all(requests.map((p) => p.catch((e) => e)));

      const successfulResults = results.filter((result) => !(result instanceof Error));

      if (successfulResults.length > 0) {
        coloredInfo(`At least one successful response`);
        coloredDebug(`Confirming jito transaction...`);
        return await this.confirm(jitoTxsignature, latestBlockhash);
      } else {
        coloredDebug(`No successful responses received for jito`);
      }

      return { confirmed: false };
    } catch (error) {
      if (error instanceof AxiosError) {
        coloredInfo(`${JSON.stringify(error.response?.data)} Failed to execute jito transaction`);
      }
      coloredError('Error during transaction execution', error as string );
      return { confirmed: false };
    }
  }
